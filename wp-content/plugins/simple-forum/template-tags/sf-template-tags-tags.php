...
Simple:Press Forum
Template Tag(s) - Tags
$LastChangedDate: 2009-07-12 12:53:40 -0700 (Sun, 12 Jul 2009) $
$Rev: 2196 $
*/
...
function sf_most_used_tags($limit=10)
{
	global $wpdb, $sfvars;
	...
	if ($sftags)
	{
		foreach ($sftags as $sftag)
		{
			...
			$out.= '<a href="'.SFURL.'?forum=all&amp;value='.$sftag->tag_slug.'&amp;type=1&amp;include=4&amp;search=1">'.$sftag->tag_name.'</a> ('.$sftag->tag_count.')'."\n";  // ******** UPDATE WITH SEARCH URL
			$out.= "</li>\n";
		}
	} else {
		$out.= "<li class='sftaglist'>\n";
...
function sf_tag_cloud($limit=25, $sort='random', $size=true, $smallest=8, $largest=22, $unit='pt', $color=true, $mincolor='#000000', $maxcolor='#cccccc')
{
	global $wpdb, $sfvars;
	...
	if ($sftags)
	{
		foreach ($sftags as $sftag)
		{
			...
			$tagout = $format;
			$tagout = str_replace('%tag_name%', wp_specialchars($sftag->tag_name), $tagout);
			$tagout = str_replace('%tag_link%', SFURL.'?forum=all&amp;value='.$sftag->tag_slug.'&amp;type=1&amp;include=4&amp;search=1', $tagout);
...
/* 	=====================================================================================
	...
	parameters:
		...
		echo			write display (returns it if false)				true/false		false
