...
add_action('admin_head', array('Post_Planner_WPAlchemy_MetaBox', '_global_head'));

add_action('admin_footer', array('Post_Planner_WPAlchemy_MetaBox', '_global_foot'));

define('POST_PLANNER_WPALCHEMY_MODE_ARRAY', 'array');

define('POST_PLANNER_WPALCHEMY_MODE_EXTRACT', 'extract');

define('POST_PLANNER_WPALCHEMY_FIELD_HINT_TEXT', 'text');

define('POST_PLANNER_WPALCHEMY_FIELD_HINT_TEXTAREA', 'textarea');

define('POST_PLANNER_WPALCHEMY_FIELD_HINT_CHECKBOX', 'checkbox');

define('POST_PLANNER_WPALCHEMY_FIELD_HINT_CHECKBOX_MULTI', 'checkbox_multi');

define('POST_PLANNER_WPALCHEMY_FIELD_HINT_RADIO', 'radio');

define('POST_PLANNER_WPALCHEMY_FIELD_HINT_SELECT', 'select');

define('POST_PLANNER_WPALCHEMY_FIELD_HINT_SELECT_MULTI', 'select_multi');

// depreciated, use WPALCHEMY_FIELD_HINT_SELECT_MULTI instead
define('POST_PLANNER_WPALCHEMY_FIELD_HINT_SELECT_MULTIPLE', 'select_multiple');

define('POST_PLANNER_WPALCHEMY_LOCK_TOP', 'top');

define('POST_PLANNER_WPALCHEMY_LOCK_BOTTOM', 'bottom');

define('POST_PLANNER_WPALCHEMY_LOCK_BEFORE_POST_TITLE', 'before_post_title');

define('POST_PLANNER_WPALCHEMY_LOCK_AFTER_POST_TITLE', 'after_post_title');

define('POST_PLANNER_WPALCHEMY_VIEW_START_OPENED', 'opened');

define('POST_PLANNER_WPALCHEMY_VIEW_START_CLOSED', 'closed');

define('POST_PLANNER_WPALCHEMY_VIEW_ALWAYS_OPENED', 'always_opened');

class Post_Planner_WPAlchemy_MetaBox
{
	/**
	 ...
	 * @var		string
	 */
	var $mode = POST_PLANNER_WPALCHEMY_MODE_ARRAY;
	...
	function Post_Planner_WPAlchemy_MetaBox($arr)
	{
		...
		if (is_array($arr))
		{
			...
			if ($this->lock_on_top) $this->lock = POST_PLANNER_WPALCHEMY_LOCK_TOP;
			elseif ($this->lock_on_bottom) $this->lock = POST_PLANNER_WPALCHEMY_LOCK_BOTTOM;
	...
	{
		if (POST_PLANNER_WPALCHEMY_MODE_ARRAY == $this->mode AND $key == $this->id)
		{
	...
	function _init()
	{
		...
		if ( ! Post_Planner_WPAlchemy_MetaBox::_is_post() AND ! Post_Planner_WPAlchemy_MetaBox::_is_page()) return;
	...
	function _foot()
	{
		...
		)
		{
			...
			(function($){ /* not using jQuery ondomready, code runs right away in footer */
				...
				<?php if (POST_PLANNER_WPALCHEMY_LOCK_TOP == $this->lock): ?>
				...
				<?php endif; ?>
				$('#'+id).append(mb);
				<?php elseif (POST_PLANNER_WPALCHEMY_LOCK_BOTTOM == $this->lock): ?>
				<?php if ('side' == $this->context): ?>
				...
				<?php endif; ?>
				$('#'+id).append(mb);
				<?php elseif (POST_PLANNER_WPALCHEMY_LOCK_BEFORE_POST_TITLE == $this->lock): ?>
				<?php if ('side' != $this->context): ?>
				...
				$('#'+id).append(mb);
				<?php endif; ?>
				<?php elseif (POST_PLANNER_WPALCHEMY_LOCK_AFTER_POST_TITLE == $this->lock): ?>
				<?php if ('side' != $this->context): ?>
				...
				<?php if (POST_PLANNER_WPALCHEMY_VIEW_START_OPENED == $this->view): ?>
				mb.removeClass('closed');
				<?php elseif (POST_PLANNER_WPALCHEMY_VIEW_START_CLOSED == $this->view): ?>
				mb.addClass('closed');
				<?php elseif (POST_PLANNER_WPALCHEMY_VIEW_ALWAYS_OPENED == $this->view): ?>
	...
	{
		if ('post' == Post_Planner_WPAlchemy_MetaBox::_is_post_or_page())
		{
			return TRUE;
		}
	...
	{
		if ('page' == Post_Planner_WPAlchemy_MetaBox::_is_post_or_page())
		{
			return TRUE;
		}
	...
	{
		$post_type = Post_Planner_WPAlchemy_MetaBox::_get_current_post_type();
	...
	static function _get_current_post_type()
	{
		...
		if ( isset( $uri ) )
		{
			...
			{
				$post_id = Post_Planner_WPAlchemy_MetaBox::_get_post_id();
	...
	{
		if (!$this->in_template AND $this->mode == POST_PLANNER_WPALCHEMY_MODE_EXTRACT)
		{
		...
		$hints = array
		(
			POST_PLANNER_WPALCHEMY_FIELD_HINT_CHECKBOX_MULTI,
			POST_PLANNER_WPALCHEMY_FIELD_HINT_SELECT_MULTI,
			POST_PLANNER_WPALCHEMY_FIELD_HINT_SELECT_MULTIPLE,
		);
	...
	function _save($post_id) 
	{
		...
		Post_Planner_WPAlchemy_MetaBox::clean($new_data);
		...
		if ($this->mode == POST_PLANNER_WPALCHEMY_MODE_EXTRACT)
		{
			$new_fields = array();
	...
	function clean(&$arr)
	{
		if (is_array($arr))
		{
			foreach ($arr as $i => $v)
			{
				if (is_array($arr[$i])) 
				{
					Post_Planner_WPAlchemy_MetaBox::clean($arr[$i]);
